    ...
    <section className="py-24 border-t border-white/5" id="services-catalog">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="mt-4  text-zinc-400 tracking-wider uppercase font-mono max-w-2xl mx-auto leading-relaxed">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-12 pb-6 border-b border-white/5">
            ...
            <div className="flex space-x-1 pl-1">
              {categories.map((cat) => (
                <button
                  ...
                  className={`px-4 py-1.5 text-[15px] font-bold uppercase tracking-widest transition-all cursor-pointer rounded-sm ${
          ...
          <div className="relative w-full md:w-80 shrink-0">
            ...
            <input
              ...
              className="w-full rounded-sm border border-white/10 bg-white/[0.02] px-9 py-2.5  text-white placeholder-zinc-500 focus:border-orange-500/50 focus:bg-[#080808] focus:outline-none transition-all"
            />
          </div>
        </div>
        ...
        {filteredServices.length === 0 ? (
          <div className="text-center py-20 border border-dashed border-white/5 rounded-sm bg-white/[0.01]">
            ...
            <p className="text-zinc-400  mt-1">Try refining your search keyword or clearing the active category filters.</p>
          </div>
        ) : (
              ...
              >
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <span className="rounded-sm bg-orange-500/10 border border-orange-500/20 px-2.5 py-0.5 text-[16px] font-bold text-orange-400 uppercase tracking-widest font-mono">
                      {service.category}
                    </span>
                    <span className="font-mono text-[16px] text-zinc-400">
                      SYS-{service.number}
                    </span>
                  </div>
                  ...
                  {/* Tagline */}
                  <p className=" text-zinc-300 leading-relaxed font-sans mb-6 line-clamp-3">
                    {service.tagline}
                  </p>
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-8">
                    {service.techStack.slice(0, 4).map((tech, idx) => (
                      <span
                        key={idx}
                        className="rounded-sm bg-white/[0.04] px-2 py-1 font-mono text-[16px] text-zinc-350 border border-white/10 shadow-sm"
                      >
                        {tech}
                      </span>
                    ))}
                    {service.techStack.length > 4 && (
                      <span className="font-mono text-[16px] text-zinc-400 pl-1 mt-1 font-semibold">
                ...
                <div className="relative z-10 border-t border-white/5 pt-4 flex items-center justify-between mt-auto">
                  <div>
                    <span className="block font-mono text-[16px] uppercase text-zinc-400 tracking-widest font-bold">Contract Budget</span>
                    <p className="font-display font-black text-white text-md">
                      ${service.pricing.oneTime}
                      <span className="text-[15px] text-zinc-400 font-normal">.00</span>
                    </p>
                  </div>
                  ...
                  <div className="flex items-center space-x-2">
                    <button
                      ...
                      className="rounded-sm border border-white/10 hover:border-orange-500/60 bg-transparent px-4 py-2 text-[16px] uppercase tracking-widest font-bold text-zinc-300 hover:text-white transition-all cursor-pointer flex items-center space-x-1 shadow-lg"
                    >
