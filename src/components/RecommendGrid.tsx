  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 animate-fadeIn" id="recommendations-hub">
      ...
      <div className="text-center max-w-3xl mx-auto mb-16">
        ...
        <p className="mt-4  font-mono uppercase tracking-wider text-zinc-400 max-w-2xl mx-auto leading-relaxed">
      ...
      <div className="flex justify-center overflow-x-auto pb-4 mb-12 border-b border-white/5 gap-2 no-scrollbar">
        {categoriesList.map((cat) => (
          <button
            ...
            className={`rounded-sm px-4 py-2 text-[15px] font-bold uppercase tracking-widest whitespace-nowrap transition-all cursor-pointer ${
              activeCategory === cat
                ...
                >
                  <div>
                    ...
                    <div className="flex items-center justify-between mb-4">
                      {item.badge ? (
                        <span className={`rounded-sm px-2.5 py-0.5 text-[15px] font-bold uppercase tracking-widest bg-orange-500/10 border border-orange-500/20 text-orange-400`}>
                          {item.badge}
                        </span>
                      ) : (
                      ...
                      <span className="font-mono text-[16px] text-zinc-400 flex items-center space-x-0.5 font-semibold">
                    ...
                    <p className="font-mono text-[16px] text-zinc-400 mt-1 uppercase tracking-widest font-bold pb-3 border-b border-white/[0.04]">
                      {item.tagline}
                    </p>

                    {/* Description */}
                    <p className="mt-4  text-zinc-300 leading-relaxed font-sans font-normal mb-5">
                      {item.description}
                    </p>

                    {/* Checklist Pros */}
                    <div className="space-y-1.5 mb-6">
                      <span className="block font-mono text-[16px] uppercase text-zinc-400 tracking-widest font-bold">Pros / Criteria</span>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-3 gap-y-1.5">
                        {item.pros.map((pro, i) => (
                          <div key={i} className="flex items-center text-[16px] text-zinc-300 font-sans">
                  ...
                  <a
                    ...
                    className="relative z-10 w-full rounded-sm border border-white/10 bg-white/[0.01] hover:bg-white/[0.04] hover:border-orange-500/40 py-3 text-center text-[16px] font-bold uppercase tracking-widest text-zinc-200 hover:text-white transition-all flex items-center justify-center space-x-1.5 duration-300"
                  >
